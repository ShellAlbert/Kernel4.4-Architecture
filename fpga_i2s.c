...
static int fpga_i2s_open(struct inode *inode,struct file*filp)
{
    ...
    val|=I2S_XFER_RXS_START;//start RX transfer.
    val|=I2S_XFER_TXS_START;//start TX transfer.
    iowrite32(val,i2s->paddr_i2s_xfer);

    filp->private_data=i2s;

    printk(KERN_INFO DRV_NAME ":open.\n");
    return 0;
}
static int fpga_i2s_release(struct inode *inode,struct file*filp)
{
    ...
    val&=(~(1<<I2S_XFER_RXS_SHIFT)|I2S_XFER_RXS_STOP);//start RX transfer.
    val&=(~(1<<I2S_XFER_TXS_SHIFT)|I2S_XFER_TXS_STOP);//start TX transfer.
    iowrite32(val,g_i2s->paddr_i2s_xfer);

    printk(KERN_INFO DRV_NAME ":release.\n");
    return 0;
}
static ssize_t fpga_i2s_read(struct file *filp,char __user *buf,size_t count,loff_t *ppos)
{
    volatile unsigned int nfifo,nfifo0,nfifo1,nfifo2,nfifo3;
    struct fpga_i2s_dev *i2s=filp->private_data;
    wait_event_interruptible(i2s->wait_queue,i2s->brx_okay);

    //read number of valid data entries.
    nfifo=ioread32(i2s->paddr_i2s_rxfifolr);
    nfifo0=(nfifo>>0)&0x3F;
    nfifo1=(nfifo>>6)&0x3F;
    nfifo2=(nfifo>>12)&0x3F;
    nfifo3=(nfifo>>18)&0x3F;
    //read data from RxFIFO.
    nfifo=nfifo0+nfifo1+nfifo2+nfifo3;
    while(nfifo-->0)
    {
        volatile unsigned int pcm;
        pcm=ioread32(i2s->paddr_i2s_rxdr);
    }
    printk(KERN_INFO DRV_NAME ":rx fifo %d\n",nfifo);
    ...
    printk(KERN_INFO DRV_NAME ":read.\n");
    return 0;
}
...
static int fpga_i2s_probe(struct platform_device *pdev)
{
    ...
    val&=~I2S_RXCR_CSR_MASK;//2'b00:two channel.
    val&=~I2S_RXCR_HWT;//0:low 16-bit for left,high 16-bit for right.
    //SJM,we donot care.
    val&=~I2S_RXCR_FBM_SHIFT;//MSB.
    val&=(~I2S_RXCR_IBM_MASK|I2S_RXCR_IBM_NORMAL);//I2S Bus Mode=I2S normal.
    val&=(~I2S_RXCR_PBM_MASK|I2S_RXCR_PBM_MODE(7));//PCM Bus Mode=PCM delay 1 mode.
    val&=(~I2S_RXCR_TFS_MASK|I2S_RXCR_TFS_I2S);//transfer format select:i2s.
    val&=(~I2S_RXCR_VDW_MASK|I2S_RXCR_VDW(32));//32-bit valid data width.
    iowrite32(val,i2s->paddr_i2s_rxcr);

    //3.config I2S_CKR.
    val=ioread32(i2s->paddr_i2s_ckr);
    val&=(~I2S_CKR_TRCM_MASK|I2S_CKR_TRCM_TXRX);//tx_lrck/rx_lrcr for Tx/Rx.
    val&=(~I2S_CKR_MSS_MASK|I2S_CKR_MSS_SLAVE);//slave mode,SCLK input.
    val&=(~I2S_CKR_CKP_MASK|I2S_CKR_CKP_NEG);//0:sample at posedge clk,drive at negedge clk.
    val&=(~(1<<I2S_CKR_RLP_SHIFT)|I2S_CKR_RLP_NORMAL);//i2s normal,low for left,high for right channel.
    val&=(~(1<<I2S_CKR_TLP_SHIFT)|I2S_CKR_TLP_NORMAL);//Transmit lrck polarity.
    val&=(~I2S_CKR_MDIV_MASK|I2S_CKR_MDIV(255));//mclk divider. Fmclk=256*Ftxsclk.
    val&=(~I2S_CKR_RSD_MASK|I2S_CKR_RSD(255));//receive sclk divider=Fsclk/Frxlrck.(255:256fs).
    val&=(~I2S_CKR_TSD_MASK|I2S_CKR_TSD(255));//transmit sclk divider=Fsclk/Ftxlrck.(255:256fs).
    iowrite32(val,i2s->paddr_i2s_ckr);

    //4.disable DMA,here we use IRQ to fetch data.
    val=ioread32(i2s->paddr_i2s_dmacr);
    val&=(~(1<<I2S_DMACR_RDE_SHIFT)|I2S_DMACR_RDE_DISABLE);//Receive DMA disabled.
    //Receive Data Level,we donot care.
    //This bit field controls the level at which a DMA request is made by the receive logic.
    val&=(~(1<<I2S_DMACR_TDE_SHIFT)|I2S_DMACR_TDE_DISABLE);//Transmit DMA disabled.
    ...
    val&=(~I2S_INTCR_RFT(32)|I2S_INTCR_RFT(32));
    val|=I2S_INTCR_RXOIC;//write 1 to clear RX overrun interrupt.
    val|=I2S_INTCR_RXOIE_ENABLE;//enable RX overrun interrupt.
    val|=I2S_INTCR_RXFIE_ENABLE;//enable RX full interrupt.

    //Transmit FIFO threshold.
    //the transmit FIFO empty interrupt is triggered.
    val&=(~I2S_INTCR_TFT(32)|I2S_INTCR_TFT(32));
    val|=I2S_INTCR_TXUIC;//write 1 to clear Tx underrun interrupt.
    val&=(~(1<<I2S_INTCR_TXUIE_SHIFT)|I2S_INTCR_TXUIE_DISABLE);//disable Tx underrun interrupt.
    val&=(~(1<<I2S_INTCR_TXEIE_SHIFT)|I2S_INTCR_TXEIE_DISABLE);//disable Tx empty interrupt.
